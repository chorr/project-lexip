...
class Pixel {
  ...
  public void update(int c, ArrayList buff) {
    int newColor;
    float h, s, b;
    ...
    if (isBreak) {
      h = hue(c);
      if (ScreenUtil.PIXEL_MODE == 0 || ScreenUtil.PIXEL_MODE == 2 || ScreenUtil.PIXEL_MODE == 6) {
  ...
  public void update(PImage target) {
    ...
    if (ScreenUtil.PIXEL_MODE == 6) {  // trinity
      ...
      for (int i=0; i < pixels.length; i++) {
        int hR = color(red(((PImage)ls.get(12)).pixels[i]), 0, 0);
        int hG = color(0, green(((PImage)ls.get(6)).pixels[i]), 0);
        int hB = color(0, 0, blue(((PImage)ls.get(0)).pixels[i]));
